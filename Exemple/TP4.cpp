...
static void display(void) {
    ...
    glPushMatrix();
    gluLookAt(-10.0, -15.0, -120.0,0.0,0.0,-100.0,0.0,2.0,0.0);
    scene();
    glPopMatrix();
    glFlush();
    glutSwapBuffers();
...
static void reshape(int tx, int ty) {
    ...
    glLoadIdentity();
    double ratio = (double)tx / (double)ty;

    float foc = asin(6.5/20) * 2 * (180 / PI);

    if (ratio >= 1.0)
    {
        gluPerspective(foc,ratio,0.1,1000);
    }
    else
    {
        gluPerspective(foc/ratio, ratio, 0.1, 1000);
    }
    ...
    if (ratio >= 1.0 )
    {
        glOrtho(-6.5*ratio, 6.5*ratio, -6.5, 6.5, 93, 107);
    }
    else
    {
        glOrtho(-6.5, 6.5, -6.5/ratio, 6.5/ratio, 93, 107);
    }
    */
